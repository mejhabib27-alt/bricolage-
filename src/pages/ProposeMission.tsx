...
const STEPS = [
  { id: 1, title: 'Client', description: 'À qui proposez-vous ?' },
  { id: 2, title: 'Service', description: 'Quel type de tâche ?' },
  { id: 3, title: 'Détails', description: 'Durée et matériel' },
  { id: 4, title: 'Budget & Lieu', description: 'Prix et adresse' },
];
...
export default function ProposeMission() {
  ...
  useEffect(() => {
    ...
    // Fetch recent conversations to get client list
    const fetchClients = async () => {
      try {
        ...
        const clients = snapshot.docs.map(doc => {
          ...
          return {
            id: otherId,
            ...data.participantDetails[otherId]
          };
        }).filter(c => c.id); // Ensure we have an ID
  ...
  const handleNext = () => {
    if (currentStep === 1 && !formData.clientId) return;
    if (currentStep === 2 && !formData.taskType && !formData.customTaskType) return;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, 'bookings'), {
        ...
        providerId: user.uid,
        providerName: profile?.name || user.displayName,
        status: 'proposed', // Specifically "proposed" by jobber
        createdAt: serverTimestamp(),
        totalAmount: formData.budget,
      });
      setSuccess(true);
      setTimeout(() => navigate('/jobber'), 3000);
    } catch (error) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-gray-50">
        ...
        >
          ...
          <h2 className="text-3xl font-display font-bold text-gray-900 mb-4">Mission proposée !</h2>
          <p className="text-gray-500 mb-8 leading-relaxed">Votre proposition a été envoyée au client. Il pourra l'accepter ou en discuter avec vous.</p>
          ...
          <p className="text-xs text-gray-400 mt-4 font-bold uppercase tracking-widest">Redirection vers votre espace jobber...</p>
        </motion.div>
      </div>
    );
  }
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-12">
          <div className="flex items-center justify-between mb-8">
            <h1 className="text-4xl font-display font-bold text-gray-900">Proposer une mission</h1>
            ...
            <div className="bg-white rounded-[48px] p-8 lg:p-12 border border-gray-100 shadow-sm min-h-[600px] flex flex-col">
              <AnimatePresence mode="wait">
                {currentStep === 1 && (
                  ...
                  >
                    <div>
                      <h3 className="text-2xl font-display font-bold text-gray-900 mb-2">À quel client proposez-vous ?</h3>
                      <p className="text-gray-500">Sélectionnez un client parmi vos contacts récents.</p>
                    </div>
                    ...
                    <div className="space-y-4">
                      {loadingClients ? (
                        ...
                        <div className="p-12 text-center bg-gray-50 rounded-[32px] border border-dashed border-gray-200">
                          ...
                          <p className="text-gray-500 font-medium">Aucun client récent trouvé.</p>
                          <p className="text-xs text-gray-400 mt-2">Discutez d'abord avec un client pour lui proposer une mission.</p>
                        </div>
                      ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          {recentClients.map((client) => (
                            <button
                              key={client.id}
                              onClick={() => setFormData(prev => ({ ...prev, clientId: client.id, clientName: client.name }))}
                            ...
                            >
                              <img 
                                src={client.photoUrl || `https://ui-avatars.com/api/?name=${client.name}`} 
                                alt={client.name} 
                                className="w-12 h-12 rounded-full object-cover"
                                referrerPolicy="no-referrer"
                              />
                              <div className="text-left">
                                <p className={`font-bold ${formData.clientId === client.id ? 'text-primary' : 'text-gray-900'}`}>{client.name}</p>
                                <p className="text-xs text-gray-400">Client récent</p>
                              </div>
                              {formData.clientId === client.id && <Check size={20} className="ml-auto text-primary" />}
                            </button>
                          ))}
                        </div>
                      )}
                ...
                {currentStep === 2 && (
                  ...
                  >
                    <div>
                      <h3 className="text-2xl font-display font-bold text-gray-900 mb-2">Quel service proposez-vous ?</h3>
                      <p className="text-gray-500">Choisissez une catégorie et un type de tâche.</p>
                    </div>
                ...
                {currentStep === 3 && (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-gray-500">Précisez les modalités techniques.</p>
                    </div>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                          {[
                            ...
                            'Le client doit fournir le matériel',
                        ...
                        <div className="space-y-4">
                          <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Outils spécifiques nécessaires</p>
                ...
                {currentStep === 4 && (
                  ...
                  >
                    <div>
                      <h3 className="text-2xl font-display font-bold text-gray-900 mb-2">Budget et Lieu</h3>
                      <p className="text-gray-500">Dernière étape pour finaliser votre proposition.</p>
                    </div>
                    ...
                    <div className="space-y-8">
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-6">
                          <label className="block text-sm font-bold text-gray-400 uppercase tracking-widest">Prix proposé au client</label>
                      ...
                      {/* Location */}
                      <div>
                        <label className="block text-sm font-bold text-gray-400 uppercase tracking-widest mb-4">Lieu de la mission</label>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                          <label className="block text-sm font-bold text-gray-400 uppercase tracking-widest mb-4">Date prévue</label>
                          <input 
                            type="date" 
                      ...
                      {/* Description */}
                      <div>
                        <label className="block text-sm font-bold text-gray-400 uppercase tracking-widest mb-4">Instructions & Notes</label>
                        <textarea 
                          placeholder="Détaillez votre proposition (matériel apporté, étapes de la mission...)"
              ...
              <div className="mt-12 pt-8 border-t border-gray-50 flex items-center justify-between">
                ...
                {currentStep < 4 ? (
                  <button
                    onClick={handleNext}
                    disabled={(currentStep === 1 && !formData.clientId) || (currentStep === 2 && !formData.taskType)}
                    className={`btn-primary px-10 py-4 rounded-2xl flex items-center gap-2 shadow-xl shadow-primary/20 ${
                      (currentStep === 1 && !formData.clientId) || (currentStep === 2 && !formData.taskType) ? 'opacity-50 cursor-not-allowed' : ''
                    }`}
                  >
                    Suivant
            ...
            <div className="bg-white rounded-[40px] p-8 border border-gray-100 shadow-sm sticky top-24">
              ...
              <div className="space-y-6">
                <div className="flex gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-gray-900">{formData.clientName || 'Non sélectionné'}</p>
                  </div>
                </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div>
                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Durée</p>
                ...
                <div className="pt-6 border-t border-gray-50">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-bold text-gray-500">Prix proposé</span>
                    <span className="text-2xl font-display font-bold text-primary">{formData.budget}€</span>
                  </div>
                  <p className="text-[10px] text-gray-400 font-medium italic">Net à payer par le client</p>
                </div>
              </div>
              ...
              <div className="mt-8 p-6 bg-blue-50 rounded-3xl border border-blue-100 flex gap-4">
                ...
                <p className="text-xs text-blue-700 font-medium leading-relaxed">
                  Cette proposition créera une mission en attente de validation par le client.
                </p>
              </div>
            </div>
          </div>
